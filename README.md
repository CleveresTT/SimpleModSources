...
Метод `isOpaqueCube` нужен, чтобы определить непрозрачность блока. Если у блока есть своя OBJ моделька, то нужно сделать блок прозрачным, задав значение false, иначе будет эффект X-Ray при просмотре через него.
